    ...
    if (ending) {
        ...
        if (endingType == "start") {
            ...
            if (endingEnd && (m_endingStack.empty() || NotInEndingStack(measure->GetN()))) {
                ...
                if (!m_tieStack.empty()) {
                    for (auto tie : m_tieStack) {
                        m_tieStack.back().first->SetEnd(measure->GetLeftBarLine());
                        m_tieStack.pop_back();
                    }
                }
            }
    ...
    pugi::xml_node node, Measure *measure, const std::string &measureNum, const short int staffOffset)
{
    ...
    if (bracket) {
        ...
        if (HasAttributeWithValue(bracket, "type", "stop")) {
            ...
            //should be line extender, CCLI fix
            if (placeStr == "below") {
                bracketSpan->SetPlace(STAFFREL_below);
            }
...
void MusicXmlInput::ReadMusicXmlHarmony(pugi::xml_node node, Measure *measure, const std::string &measureNum)
{
    ...
    if (kind) {
        if (HasAttributeWithValue(kind, "use-symbols", "yes")) {
            ...
            harmText = kind.attribute("text").as_string();
        }
        else {
...
// CCLI fixes
        if (!m_lyricExtensionStack.empty()) {
            if ((node.child("lyric") == NULL) && ((nextNote.node().child("lyric") != NULL) || (nextNote.node().child("rest") != NULL))) {
                std::vector<int> representedVerseNumbers = {};
                bool numVerses = 0;
                for (pugi::xml_node nextNoteChild : nextNote.node().children()) {
                    if (strcmp(nextNoteChild.name(), "lyric") == 0) {
                        representedVerseNumbers.push_back(nextNoteChild.attribute("number").as_int());
                    }
                }
                for (auto extendedSyl : m_lyricExtensionStack) {
                    int verseNumber = extendedSyl.first.first;
                    ...
                    bool doesSongOnlyHaveOneVerse = (representedVerseNumbers.size() == 1) && (std::none_of(representedVerseNumbers.begin(), representedVerseNumbers.end(), [](int i){return i>1;}));
                    if (!didMatchLyrics && doesSongOnlyHaveOneVerse) {
                        m_lyricExtensionStack.back().second->SetEnd(note); //ONLY FOR NONSTARTER LEs
                        m_lyricExtensionStack.pop_back();
                    }
                }
    ...
    const pugi::xml_node &node, Layer *layer, Note *note, const std::string &measureNum)
{
    ...
    if ("stop" == tieType) { // add to stack if (endTie) or if pitch/oct match to open tie on m_tieStack
        if (!m_tieStack.empty()) {
            ...
            double tstamp = std::min(static_cast<double>(m_layerEndTimes[layer]), static_cast<double>(m_durTotal));
            tstamp = std::max(tstamp, static_cast<double>(m_durTotal));
